...
1. What are the main differences between a stateful and a functional component?
  ...
  Stateful components require a lifecycle method. These lifecycle methods need a constructor plus super() to handle state and a render method to retun. Props are also passed using the syntaxt of 'this.props' because they are class components.

  Functional components use hooks to deal with the lifecycle and state. They only need a return and pass props as an argument.

2. When does a componentWillMount function be called? What about a componentWillUpdate?

  componentWillMount is called right before the render method in the lifecycle.

  componentWillUpdate is called every time a re-render is required, such as when this.setState() is called. It allows us to access the new props/state.

3. Define stateful logic.

  Any logic that affects non-visual aspects of code and helps manage the state of components.

4. What are the three step of creating a successful test? What is done in each phase?

Arrange, Act, Assert. Arrange uses the render method to grab a react component. Act allows us to trigger some events or inspect certain lines of code. Assert ensures the code acts as expected.
